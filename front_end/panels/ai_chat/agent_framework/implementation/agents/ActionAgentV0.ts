...
/**
 * V0 (Baseline) version of ActionAgent
 *
 * Extracted from git commit 634a6b2f70 (original implementation).
 *
 * Key differences from current (v1):
 * - 10 max iterations (vs 12 in v1)
 * - No EncodedId format guidance in system prompt
 * - No focus selector method
 * - No searchQuery usage guidance
 * - Simpler page change analysis
 * - Uses get_page_content_v0 instead of get_page_content
 *
 * Used for A/B comparison to measure improvements.
 */
...
/**
 * Create the configuration for the V0 Action Agent (baseline)
 */
export function createActionAgentV0Config(): AgentToolConfig {
  return {
    name: 'action_agent_v0',
    version: AGENT_VERSION,
    description: 'V0 BASELINE: Executes a single, low-level browser action with enhanced targeting precision (such as clicking a button, filling a field, selecting an option, or scrolling) on the current web page, based on a clear, actionable objective. ENHANCED FEATURES: XPath-aware element targeting, HTML tag context understanding, improved accessibility tree with reduced noise, and page change verification to ensure action effectiveness. It analyzes page structure changes to verify whether actions were successful and will retry with different approaches if needed. Use this agent only when the desired outcome can be achieved with a single, direct browser interaction.',
...
## Process Flow
1. When given an objective, first analyze the page structure using get_page_content_v0 tool to access the enhanced accessibility tree or use extract_data to extract the specific element you need to interact with
...
- perform_action with method='setChecked' for specific checkbox state: { "method": "setChecked", "nodeId": 789, "args": { "checked": true } }`,
    ...
    maxIterations: 10,  // Original v0 had 10 iterations (v1 has 12)
    ...
    handoffs: [
      {
        ...
        includeToolResults: ['perform_action', 'get_page_content_v0']
      }
    ],
  };
}
