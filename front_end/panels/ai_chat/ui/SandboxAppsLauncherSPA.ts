...
/**
 * Sandbox Apps Launcher SPA - Bundled HTML, CSS, and JS for the launcher web app
 *
 ...
 * - SPA → DevTools: window.__sandboxAppsLauncherBridge(payload) (via Runtime.addBinding)
 * - DevTools → SPA: window.miniApp.dispatch(action) (via Runtime.evaluate)
 */

export const SandboxAppsLauncherSPA = {
  html: getHTML(),
  css: getCSS(),
  js: getJS(),
};
  ...
  <div class="launcher">
    ...
    <header class="launcher-header">
      <div class="header-left">
        ...
        <h1 class="launcher-title">Sandbox Apps</h1>
      </div>
...
function getJS(): string {
  return `
      ...
      if (apps.length === 0) {
        grid.innerHTML = \`
          <div class="empty-state">
            ...
            <p class="empty-state-message">Sandbox apps will appear here when they are registered.</p>
          </div>
        \`;
        return;
      }
    ...
    function sendToDevTools(message) {
      if (typeof window.__sandboxAppsLauncherBridge === 'function') {
        window.__sandboxAppsLauncherBridge(JSON.stringify(message));
      } else {
