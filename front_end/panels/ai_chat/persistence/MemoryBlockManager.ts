...
// Constants
const MEMORY_SESSION_ID = '__global_memory__';
const BLOCK_LIMITS = {
  user: 20000,
  facts: 20000,
  project: 20000,
...
export class MemoryBlockManager {
  ...
  private async withGlobalSession<T>(fn: () => Promise<T>): Promise<T> {
    ...
    this.fileManager.setSessionId(MEMORY_SESSION_ID);
    try {
      return await fn();
    } finally {
      ...
      if (exists) {
        ...
        if (type === 'project') {
          ...
          if (projects.length >= MAX_PROJECT_BLOCKS) {
            throw new Error(`Max ${MAX_PROJECT_BLOCKS} project blocks allowed`);
          }
        }
  ...
  /**
   * Get the character limit for a block type.
   */
  static getBlockLimit(type: BlockType): number {
   ...
   */
  static getMaxProjectBlocks(): number {
    return MAX_PROJECT_BLOCKS;
  }
}
